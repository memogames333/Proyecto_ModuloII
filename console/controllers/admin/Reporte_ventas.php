...
class Reporte_ventas extends CI_Controller {
	...
	public function index()
	{
		...
		$data = array(
			...
			"uri"=>"admin/reporte_ventas/generar"
		);
		$extras = array(
			'css' => array(),
			'js' => array(),
	...
	public function generar(){
		...
		$this->fpdf->Cell(280, 5, utf8_decode("REPORTE DE VENTAS"), 0, 1, "C");
		...
		$this->fpdf->Cell(30, 5, utf8_decode("Numero de Orden"), 1, 0, "C", 1);
		$this->fpdf->Cell(70, 5, utf8_decode("Cliente"), 1, 0, "C", 1);
		$this->fpdf->Cell(50, 5, utf8_decode("Entrega"), 1, 0, "C", 1);
		$this->fpdf->Cell(45, 5, utf8_decode("Fecha Hora"), 1, 0, "C", 1);
		...
		$rows = $this->reporte->get_ventas_general(Y_m_d($desde),Y_m_d($hasta));
		...
		if ($rows != 0) {
			...
			foreach ($rows as $row) {
				...
				if($row->finalizada==1) $estado ="Finalizada";
				else $estado ="En proceso";

				$array_data = array(
					0 => array($n,5,10,"C"),
					1 => array($row->numero_orden,10,30,"L"),
					2 => array($row->usuario,30,70,"L"),
					3 => array($row->entrega,20,50,"L"),
					4 => array(d_m_Y($row->fecha)." ".hora_A_P($row->hora),40,45,"L"),
					6 => array($row->tipo,20,25,"C"),
					7 => array("$".dinero($row->envio),5,15,"R"),
					8 => array("$".dinero($row->total),20,20,"R"),
